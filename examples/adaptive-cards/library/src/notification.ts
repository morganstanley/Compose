...
export class ToastNotification {
  ...
  constructor(
    ...
    ToastNotification.notifications.push({ count: this.count, card: this.card });
  }
  ...
  renderCard() {
    if (!window.opener) {
      ...
      if (window.name === `${this.count}`) {
        let result = this.card.render(window.document.body);

        this.renderStyles(window.document);
  ...
  renderStyles(document: Document) {
    ...
    if (this.template) {
      styles.replaceSync(defaultTemplateStyles);
    } else {
